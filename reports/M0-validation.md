...
- a self-contained Pong View running inside the real `dsh-uni-editor` double iframe;
...
| `dsh-uni-editor` checks | TypeScript, build and 3 package tests | PASS |
...
The final fixture sets the existing `dsh-uni-editor.maxBodyBytes` option to 2 MiB. The Host remains bounded; no size validation was removed.
...
M0 changes the generic wrapper to `html, body { width: 100%; height: 100% }`. The final E2E proved equal outer and inner dimensions. This is the only `dsh-uni-editor` source adjustment; it adds no Three.js behavior and no fullscreen support.
